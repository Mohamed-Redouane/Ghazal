...
const HeroSection = () => {
  ...
  const changeSlide = useCallback((newIndex: number, userTriggered = false) => {
  ...
  useEffect(() => {
    ...
    if (currentSlideData.type === "video") {
      ...
      if (video && isVideoPlaying) { // Only listen for video end if it's playing
    ...
    >
      ...
      <div className="relative h-full flex items-center">
        {slides.map((slide, index) => (
          ...
          >
            {slide.type === "video" ? (
              // Fixed Responsive Video Style with proper top padding
              <div className="absolute inset-0 w-full h-full flex items-center justify-center pt-20 sm:pt-24 pb-4 px-4 sm:px-6 lg:px-8">
                <div className="relative w-full h-full max-w-6xl">
                  ...
                  {/* Main Video Container - Fully Responsive with proper aspect ratio */}
                  <div className="relative w-full h-full bg-black rounded-lg sm:rounded-xl overflow-hidden shadow-xl sm:shadow-2xl shadow-gold/20 border border-gold/10">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-black/20 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="flex items-center space-x-3 sm:space-x-4 lg:space-x-6 bg-black/70 backdrop-blur-md rounded-lg sm:rounded-xl px-4 sm:px-6 lg:px-8 py-2 sm:py-3 lg:py-4 shadow-lg sm:shadow-xl border border-gold/20">
                        ...
                        >
                          {isVideoPlaying ? <Pause size={20} className="sm:w-6 sm:h-6 lg:w-7 lg:h-7" /> : <Play size={20} className="sm:w-6 sm:h-6 lg:w-7 lg:h-7" />}
                        </button>
                        <button
                        ...
                        >
                          {isVideoMuted ? <VolumeX size={20} className="sm:w-6 sm:h-6 lg:w-7 lg:h-7" /> : <Volume2 size={20} className="sm:w-6 sm:h-6 lg:w-7 lg:h-7" />}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="absolute top-2 sm:top-4 lg:top-6 left-2 sm:left-4 lg:left-6 z-10 pointer-events-none">
                      <div className="bg-black/60 backdrop-blur-sm px-2 sm:px-3 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-md sm:rounded-lg border border-gold/30">
                        <span className="text-gold-premium font-accent text-xs sm:text-sm md:text-base lg:text-lg tracking-wide font-semibold">Ghazal Restaurant</span>
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-2 sm:bottom-4 lg:bottom-6 left-2 sm:left-4 lg:left-6 right-2 sm:right-4 lg:right-6 z-10 pointer-events-none">
                      <div className="bg-gradient-to-r from-black/70 to-black/50 backdrop-blur-sm px-3 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 rounded-md sm:rounded-lg border border-gold/20">
                        <h3 className="text-white font-display text-sm sm:text-lg md:text-xl lg:text-2xl font-bold mb-0.5 sm:mb-1">{slide.title}</h3>
                        <p className="text-gold-premium font-accent text-xs sm:text-sm md:text-base lg:text-lg italic mb-0.5 sm:mb-1">{slide.subtitle}</p>
              ...
              // Enhanced image slides with better loading states
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between h-full py-8 lg:py-16 gap-8 lg:gap-12">
                  ...
                  <div className="flex-1 text-center lg:text-left space-y-4 sm:space-y-6 lg:space-y-8 max-w-xl lg:max-w-2xl order-2 lg:order-1">
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      {/* Enhanced Arabic Slogan */}
                      {slide.slogan && (
                        <div className="relative mb-8 animate-reveal-up" style={{ animationDelay: '0.3s' }}>
                          <p className="text-gold-premium font-display font-bold text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl text-center tracking-wide leading-relaxed animate-text-glow" 
                          ...
                          <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-24 h-px bg-gradient-to-r from-transparent via-gold/50 to-transparent animate-shimmer-gold" style={{ animationDelay: '1s' }}></div>
                        </div>
                      )}
                      
                      <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-display font-bold text-white leading-tight animate-reveal-up" style={{ animationDelay: '0.5s' }}>
                        {slide.title}
                      </h1>
                      
                      <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-accent text-gold-premium italic animate-reveal-scale" style={{ animationDelay: '0.7s' }}>
                        {slide.subtitle}
                      </h2>
                      
                      <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-white/90 font-body leading-relaxed max-w-md lg:max-w-lg mx-auto lg:mx-0 animate-reveal-up" style={{ animationDelay: '0.9s' }}>
                        {slide.description}
                      </p>
                    </div>
...
>
  Découvrir le menu
                      ...
                      <Button
                        variant="outline"
                        size="lg"
                        className="glass-effect text-white hover:bg-white/10 font-body font-medium px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base lg:text-lg rounded-xl transition-all duration-300 border-2 border-white/30 hover:border-white/60 w-full sm:w-auto hover:scale-105"
                  ...
                  {/* Enhanced Right Image with Loading States */}
                  <div className="flex-1 flex justify-center items-center relative order-1 lg:order-2 w-full max-w-md sm:max-w-lg lg:max-w-none">
                    <div className="relative w-full h-48 sm:h-64 md:h-72 lg:h-80 xl:h-96 2xl:h-[28rem] flex items-center justify-center">
                      ...
                      <div className="absolute top-1/2 sm:top-1/3 lg:-bottom-8 -left-4 sm:-left-8 lg:-left-12 w-4 sm:w-8 lg:w-14 xl:w-20 h-4 sm:h-8 lg:h-14 xl:h-20 bg-teal/20 rounded-full blur-md sm:blur-lg lg:blur-xl animate-float-gentle" style={{ animationDelay: '1s' }}></div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Carousel Dots */}
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 sm:space-x-3 z-10">
        {slides.map((_, index) => (
          <button
            key={index}
