      ...
      {contactInfo.map((info, index) => {
        const CardContent = (
          <div className="text-center">
            <div
              className={`w-20 h-20 mx-auto mb-4 rounded-xl ${info.isUberEats ? "bg-white/10 p-1" : `bg-gradient-to-r ${info.gradient} p-3`} group-hover:scale-110 transition-transform duration-300 flex items-center justify-center`}
            >
              {info.isUberEats ? (
                <img
                ...
                <info.icon className="w-8 h-8 text-white stroke-[1.5]" />
              )}
            </div>
